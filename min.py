...
def customer_behaviour(name, environment, boxes, boxes_stats):
    ...
    min_idx = queues_lengths.index(min(queues_lengths))
    global total_latency
    with boxes[min_idx].request() as box:
        print >> log_fd, cur_time(environment), 'Покупатель #', name, 'встал в очередь в кассу', min_idx
        start = environment.now
        yield box
        total_latency += environment.now - start
        print >> log_fd, cur_time(environment), 'Покупатель #', name, 'обслуживается на кассе', min_idx
        yield environment.timeout(random.normalvariate(5, 0.2) * purchases_count)  # в секундах
        boxes_stats[min_idx] += 1
